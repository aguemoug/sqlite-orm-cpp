# CPP-SQLite-ORM
*A Lightweight, Compile-Time ORM Code Generator for SOCI C++ and SQLite*
...
- 🗄️ **Optimized for SQLite and SOCI** — built on top of `soci`.  
- 🧰 **clang-format Integration** — automatically formats generated code.  
- 🧰 **Auto generate << operator overload** — automatically generates stream output operators for easy debugging.  
...
// TABLE(name) marks a struct as a database table
// VIEW(name) marks a struct as a database view

// PK marks primary key fields
// AUTOINC marks an auto-incrementing primary keys  
// WIDTH(n) sets the maximum width for string in iostream output

// FK marks foreign keys
// READONLY marks view-only OR computed fields
// IGNORE marks structs or fields to be excluded from generation
// Example entity definitions:
  
struct  TABLE(users) User {
  WIDTH(10) PK AUTOINC int id;
  std::string username;
  std::string email;
  int age;
  long created_at;
...
```bash
python odb.py entities.h -o output.hpp 
```

### 3️⃣ Example of generated code
See the example code in [output.hpp](output.hpp) for generated CRUD operations.
```cpp
